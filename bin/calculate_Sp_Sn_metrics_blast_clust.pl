...
my ($assemblyDir,$individualMsf,$skip,$v,$database,$VERBOSE,$contigFileName,$PERCENTRANGE,$MINPERCENT,$blastClustFile);
my @options = (
	'a=s',		\$assemblyDir,
	'i=s',		\$individualMsf,
	's=s',		\$skip,
...
# waterBLAST the allcontigs set from the assembly
my $contigsWatered;

# waterBLAST the individual MSF
my $individualMsfWatered;

# skip the wateredBLAST step if instructed to do so
if ($skip){
	$contigsWatered = $contigs . '.wateredBLASTN';
	$individualMsfWatered = $individualMsf . '.wateredBLASTN';
}else{
	$contigsWatered = waterBLAST($contigs,$database,$v);
	$individualMsfWatered = waterBLAST($individualMsf,$database,$v);
}

# get the information about which tigs should be substituted based on the BLAST clust resutls
my $tigLengths = getLengths($contigs . '.seqclean');
my ($tigs2skip,$tigs2add) = parseBlastClust($blastClustFile,$tigLengths);
...
foreach my $tig (sort {
	...
	if ($tigs2skip->{$tig}) {
		warn "Skipping $tig as it is subsumed in the seqclean + blastclust process"; 
		next;
	}
	...
	if (defined $tig2Taxon->{$tig}){
		...
		($Sp,$Sn) = calculateSpSn($tig,$tig2Taxon,$read2Taxon,$toc,$tigs2add,$tigs2skip,$tigLengths);
	...
	foreach my $additionalTig (@{$tigs2add->{$tig}}){
		$numReads += keys %{$toc->{tig2Reads}->{$additionalTig}};
	}	
...
sub parseBlastClust {
	my $blastClust = shift;
	die "Error lost blastclust file" unless defined $blastClust;
	my $tigLength = shift;
	die "Error lost contig lengths" unless defined $tigLength;

	my (%IDs2skip,%IDs2Add);

	open(FILE,"<$blastClust") or die "Error opening $blastClust for reading: $!";
	while(<FILE>){
		chomp $_;
		my @ids = split(/\s+/,$_);
		next unless (@ids > 1);
		my $largest_id = pop @ids;
		my @smaller_ids;
		foreach my $id (@ids){
			if ($tigLength->{$id} > $tigLength->{$largest_id}){
				push(@smaller_ids,$largest_id);
				$largest_id = $id;
			}else{
				push(@smaller_ids,$id);
			}
		}
		$IDs2Add{$largest_id} = \@smaller_ids;
		foreach my $id (@smaller_ids){
			$IDs2skip{$id}++;
		}
	}
	close FILE or die "Error closing $blastClust: $!";
	return \%IDs2skip, \%IDs2Add;
}
...
sub calculateSpSn {
	...
	# Additional arguments for blastclust reduced representation
	my $tigs2add = shift;
	my $tigs2skip = shift;
	my $tigLengths = shift;
	...
	# add on any additional tigs based on the blastclust results
	foreach my $additionalTig (@{$tigs2add->{$tig}}){
		foreach my $read (keys %{$toc->{tig2Reads}->{$additionalTig}}){
			# does the read agree with the tig
			my $agree = 0;
			foreach my $m (@{$read2Taxon->{$read}->{matches}}){
				if ($tigTaxon{$m->{taxon}}){
		...
		warn "Looking for data to $additionalTig for $tig";
	}	
...
#	foreach my $otherTig (keys %{$tig2Taxon}){
	foreach my $otherTig (keys %{$tigLengths}){
		next if $otherTig eq $tig; # skip self
		next if (defined $tigs2skip and ($tigs2skip->{$otherTig} > 0));
		my $tigAgree = 0;
...
sub usage {
	die "Usage: $0 -a assemblyDir -i individualMsf -s skipAndUseThisMsf -v 5 -d database -bc /path/to/blastclust.file";
}
